...
i2swrite:
    ...
    // Update VU from the portion actually handed to I2S. Decoder output blocks
    // vary greatly (especially MP3 versus FLAC); following DMA-sized portions
    // keeps the meter cadence independent of the codec block size.
    if (settings.VU_LEVEL && m_plCh.i2s_bytesConsumed) {
        int32_t* vuBuffer = (m_output_sr && m_output_sr != m_i2s_items.sampleRate) ? m_resamplesBuff.get() : m_outBuff.get();
        const size_t framesConsumed = m_plCh.i2s_bytesConsumed / BYTES_PER_STEREOFRAME;
        for (size_t i = 0; i < framesConsumed; ++i) calculateVUlevel(vuBuffer + m_plCh.count + i * 2);
    }
...
void Audio::calculateVUlevel(int32_t* sample) { // Envelope-Follower
    const uint8_t l = std::min<uint32_t>(abs(sample[LEFTCHANNEL] >> 23), UINT8_MAX);
    const uint8_t r = std::min<uint32_t>(abs(sample[RIGHTCHANNEL] >> 23), UINT8_MAX);

    // Spectrum work is intentionally skipped when SPECTRUM is disabled.
    if (settings.SPECTRUM) {
        float mono = 0.5f * (float)((sample[LEFTCHANNEL] >> 20) + (sample[RIGHTCHANNEL] >> 20));
        constexpr float TARGET = 0.1f;
        constexpr float ATTACK = 0.05f;
        constexpr float RELEASE_FFT = 0.005f;
        const float level = fabsf(mono);
        if (level > 1e-6f) {
            const float desired = TARGET / level;
            const float factor = desired < m_fft_items.gain ? ATTACK : RELEASE_FFT;
            m_fft_items.gain += factor * (desired - m_fft_items.gain);
        }
        if (fabsf(mono) < 1e-4f) mono = 0.0f;
        m_fft_items.buffer[m_fft_items.buffer_index++] = mono * m_fft_items.gain;
        if (m_fft_items.buffer_index == m_fft_items.SIZE) m_fft_items.buffer_index = 0;
    }

    // Preserve the character of the previous yoRadio VU: a fast per-sample
    // envelope, rather than a codec-block or RMS average.
    m_vu_items.left = l >= m_vu_items.left ? (m_vu_items.left + l) * 0.5f
                                            : (m_vu_items.left * 7.0f + l) * 0.125f;
    m_vu_items.right = r >= m_vu_items.right ? (m_vu_items.right + r) * 0.5f
                                              : (m_vu_items.right * 7.0f + r) * 0.125f;
    m_vu_window_peak_left = std::max(m_vu_window_peak_left, l);
    m_vu_window_peak_right = std::max(m_vu_window_peak_right, r);
    ++m_vu_window_samples;

    const uint16_t observed = std::max<uint16_t>(std::max<uint16_t>(m_vu_items.left, m_vu_items.right),
                                                  std::max(m_vu_window_peak_left, m_vu_window_peak_right));
    if (observed > m_vu_reference.load(std::memory_order_relaxed)) {
        m_vu_reference.store(observed, std::memory_order_release);
    }

    const uint32_t windowMs = m_vu_window_ms.load(std::memory_order_relaxed);
    const uint16_t windowSamples = std::max<uint32_t>(1, static_cast<uint64_t>(m_i2s_items.sampleRate) * windowMs / 1000);
    if (m_vu_window_samples < windowSamples) return;
    const uint16_t peakHoldFrames = std::max<uint32_t>(1, settings.PEAK_HOLD_SAMPLES / windowSamples);

    if (m_vu_window_peak_left >= m_vu_items.left_peak) {
        m_vu_items.left_peak = m_vu_window_peak_left;
        m_vu_items.left_hold = peakHoldFrames;
    } else if (m_vu_items.left_hold) {
        --m_vu_items.left_hold;
    } else {
        m_vu_items.left_peak = m_vu_items.left_peak > settings.PEAK_RELEASE ? m_vu_items.left_peak - settings.PEAK_RELEASE : 0;
    }
    if (m_vu_window_peak_right >= m_vu_items.right_peak) {
        m_vu_items.right_peak = m_vu_window_peak_right;
        m_vu_items.right_hold = peakHoldFrames;
    } else if (m_vu_items.right_hold) {
        --m_vu_items.right_hold;
    } else {
        m_vu_items.right_peak = m_vu_items.right_peak > settings.PEAK_RELEASE ? m_vu_items.right_peak - settings.PEAK_RELEASE : 0;
    }

    const uint8_t write = m_vu_frame_write.load(std::memory_order_relaxed);
    const uint8_t next = (write + 1) % VU_FRAME_QUEUE_SIZE;
    uint8_t read = m_vu_frame_read.load(std::memory_order_acquire);
    if (next == read) {
        read = (read + 1) % VU_FRAME_QUEUE_SIZE;
        m_vu_frame_read.store(read, std::memory_order_release);
    }
    m_vu_frames[write] = {static_cast<uint8_t>(m_vu_items.left), static_cast<uint8_t>(m_vu_items.right), m_vu_items.left_peak, m_vu_items.right_peak};
    m_vu_frame_write.store(next, std::memory_order_release);

    m_vu_window_samples = 0;
    m_vu_window_peak_left = 0;
    m_vu_window_peak_right = 0;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::setVUSettings(uint16_t gain, uint16_t windowMs, uint16_t attackMs, uint16_t releaseMs,
                          uint16_t peakHoldMs, uint16_t peakReleaseMs) {
    m_vu_gain.store(std::clamp<uint16_t>(gain, 25, 300), std::memory_order_relaxed);
    m_vu_window_ms.store(std::clamp<uint16_t>(windowMs, 5, 50), std::memory_order_relaxed);
    m_vu_attack_ms.store(std::min<uint16_t>(attackMs, 500), std::memory_order_relaxed);
    m_vu_release_ms.store(std::clamp<uint16_t>(releaseMs, 20, 1500), std::memory_order_relaxed);
    m_vu_peak_hold_ms.store(std::min<uint16_t>(peakHoldMs, 1000), std::memory_order_relaxed);
    m_vu_peak_release_ms.store(std::clamp<uint16_t>(peakReleaseMs, 100, 3000), std::memory_order_relaxed);
}
// yoRadio compatibility: the UI consumes live levels and peak markers
// independently. Both values use the legacy inverted, dimension-scaled
// representation (silence == dimension, full scale == 0).
uint16_t Audio::get_VUlevel(uint16_t dimension) {
    dimension = std::min<uint16_t>(dimension, UINT8_MAX);
    const uint32_t now = millis();
    if (!m_f_running) {
        m_vu_frame_read.store(m_vu_frame_write.load(std::memory_order_acquire), std::memory_order_release);
        m_vu_display_frame = {};
        m_vu_display_left = 0;
        m_vu_display_right = 0;
        m_vu_display_peak_left = 0;
        m_vu_display_peak_right = 0;
        m_vu_display_peak_hold_left = 0;
        m_vu_display_peak_hold_right = 0;
        m_vu_frame_ms = now;
        m_vu_display_ms = now;
    } else {
        const uint32_t frameMs = m_vu_window_ms.load(std::memory_order_relaxed);
        uint32_t framesDue = m_vu_frame_ms ? (now - m_vu_frame_ms) / frameMs : 1;
        if (framesDue) {
            m_vu_frame_ms = m_vu_frame_ms ? m_vu_frame_ms + framesDue * frameMs : now;
            uint8_t read = m_vu_frame_read.load(std::memory_order_relaxed);
            const uint8_t write = m_vu_frame_write.load(std::memory_order_acquire);
            while (framesDue-- && read != write) {
                m_vu_display_frame = m_vu_frames[read];
                read = (read + 1) % VU_FRAME_QUEUE_SIZE;
            }
            m_vu_frame_read.store(read, std::memory_order_release);
        }
        const uint32_t elapsed = m_vu_display_ms ? std::min<uint32_t>(now - m_vu_display_ms, 100) : 16;
        m_vu_display_ms = now;
        const uint32_t attackMs = m_vu_attack_ms.load(std::memory_order_relaxed);
        const uint32_t releaseMs = m_vu_release_ms.load(std::memory_order_relaxed);
        const uint32_t peakHoldMs = m_vu_peak_hold_ms.load(std::memory_order_relaxed);
        const uint32_t peakReleaseMs = m_vu_peak_release_ms.load(std::memory_order_relaxed);
        const float attackAlpha = attackMs ? std::min(1.0f, static_cast<float>(elapsed) / attackMs) : 1.0f;
        const float releaseAlpha = std::min(1.0f, static_cast<float>(elapsed) / releaseMs);
        const auto follow = [attackAlpha, releaseAlpha](float current, float target) {
            const float alpha = target > current ? attackAlpha : releaseAlpha;
            return current + (target - current) * alpha;
        };
        m_vu_display_left = follow(m_vu_display_left, m_vu_display_frame.left);
        m_vu_display_right = follow(m_vu_display_right, m_vu_display_frame.right);
        const auto followPeak = [now, elapsed, peakHoldMs, peakReleaseMs](float live, float& peak, uint32_t& holdUntil) {
            if (live >= peak) {
                peak = live;
                holdUntil = now + peakHoldMs;
            } else if (static_cast<int32_t>(now - holdUntil) >= 0) {
                peak = std::max(live, peak - 255.0f * elapsed / peakReleaseMs);
            }
        };
        followPeak(m_vu_display_left, m_vu_display_peak_left, m_vu_display_peak_hold_left);
        followPeak(m_vu_display_right, m_vu_display_peak_right, m_vu_display_peak_hold_right);
    }
    const uint32_t reference = std::max<uint16_t>(1, m_vu_reference.load(std::memory_order_acquire));
    const uint32_t gain = m_vu_gain.load(std::memory_order_relaxed);
    const uint32_t scaledLeft = std::min<uint32_t>(reference, static_cast<uint32_t>(m_vu_display_left) * gain / 100);
    const uint32_t scaledRight = std::min<uint32_t>(reference, static_cast<uint32_t>(m_vu_display_right) * gain / 100);
    const uint8_t left = dimension - (scaledLeft * dimension / reference);
    const uint8_t right = dimension - (scaledRight * dimension / reference);
...
uint16_t Audio::get_VUpeak(uint16_t dimension) {
    ...
    const uint32_t reference = std::max<uint16_t>(1, m_vu_reference.load(std::memory_order_acquire));
    const uint32_t gain = m_vu_gain.load(std::memory_order_relaxed);
    const uint32_t scaledLeft = std::min<uint32_t>(reference, static_cast<uint32_t>(m_vu_display_peak_left) * gain / 100);
    const uint32_t scaledRight = std::min<uint32_t>(reference, static_cast<uint32_t>(m_vu_display_peak_right) * gain / 100);
    const uint8_t left = dimension - (scaledLeft * dimension / reference);
    const uint8_t right = dimension - (scaledRight * dimension / reference);
    return (static_cast<uint16_t>(left) << 8) | right;
}
